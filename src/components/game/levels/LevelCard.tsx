...
const LevelCard: React.FC<LevelCardProps> = ({ level }) => {
  ...
  const getBatteryIcon = (id: number) => {
    const icons = [Battery, BatteryLow, BatteryMedium, BatteryFull, BatteryWarning, BatteryCharging];
    ...
    >
      ...
      >
          ...
          <div className={`h-full rounded-lg p-6
            ...
            {isUnlocked ? (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 
                ...
                {isCompleted ? 'Completed - Click to replay' : 'Hover to see details'}
              </div>
            ) : (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 
        ...
        <div className="absolute inset-0 backface-visibility-hidden rotate-y-180">
          <div className="h-full bg-slate-800/50 backdrop-blur-sm rounded-lg p-6
            border border-slate-700/50 group-hover:border-emerald-500/50
            flex flex-col items-center justify-center transition-all duration-300">
            <div className="text-center flex-grow flex items-center justify-center">
              <p className="text-2xl font-medium bg-gradient-to-r from-blue-300 via-emerald-300 to-blue-300 
            ...
            >
              {isCompleted ? 'Replay Level' : isUnlocked ? 'Start Diagnosis' : 'Locked'}
            </button>
          </div>
        </div>
      </div>
